...
import io.cordova.ifb.adapter.DummyUpdationAdapter;
import io.cordova.ifb.module.DummyProductModel;
import io.cordova.ifb.utility.AppController;
import io.cordova.ifb.utility.PrefManager;
import io.cordova.ifb.utility.RecyclerItemClickListener;

public class DummyUpdationActivity extends AppCompatActivity implements RecyclerItemClickListener.OnItemClickListener{
    ArrayList<DummyProductModel> itemList=new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dummy_updation);
        initialize();
        getItemlist();
        setAdapter();
        onClick();
    ...
    private void initialize(){
        prefManager=new PrefManager(DummyUpdationActivity.this);
        rvReport=(RecyclerView)findViewById(R.id.rvReport);
        rvReport.addOnItemTouchListener(new RecyclerItemClickListener(DummyUpdationActivity.this, DummyUpdationActivity.this));
        LinearLayoutManager layoutManager
                = new LinearLayoutManager(DummyUpdationActivity.this, LinearLayoutManager.VERTICAL, false);
    ...
    private void getItemlist(){
        ...
        String surl =  AppController.APIURL+"api/get_EmployeeTemporarySales?UserID="+prefManager.getUserId()+"&FinancialYear="+financialYear+"&Month="+month+"&SecurityCode="+prefManager.getSecurityCode();
                        ...
                        try {
                            ...
                            if (responseStatus) {
                                ...
                                for (int i = 0; i < responseData.length(); i++) {
                                    ...
                                    String ModelCode=obj.optString("ModelID");
                                    ...
                                    String TempNo=obj.optString("TempNo");
                                    ...
                                    DummyProductModel obj2 = new DummyProductModel(_SalesDate,TicketNo,TempNo,ModelName,ModelCode,CustomerName,CustomerPhNo,TokenNo,CategoryName,Title,CustomerPinCode,FirstName,LastName,CustomerAlternateNumber,CustomerPhNo,CustomerEmail,HouseNo,StreetName,Landmark,Rate,ModelCode);
                                    itemList.add(obj2);
                            ...
                            e.printStackTrace();
                            Toast.makeText(DummyUpdationActivity.this, "Volly Error", Toast.LENGTH_LONG).show();
                        }
        ...
        RequestQueue requestQueue = Volley.newRequestQueue(DummyUpdationActivity.this);
        requestQueue.add(stringRequest);
    }

    private void setAdapter(){
        DummyUpdationAdapter sAdpater=new DummyUpdationAdapter(itemList);
    ...
    private void onClick(){
        ...
        imgHome.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(DummyUpdationActivity.this,DashBoardActivity.class);
    ...
    private void searchDialog() {
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(DummyUpdationActivity.this, R.style.CustomDialogNew);
    ...
    public void onItemClick(View childView, int position) {
        String tokenno=itemList.get(position).getTokenno();
        String tempno=itemList.get(position).getTemptoken();
        String CategoryName=itemList.get(position).getCategoryName();
        String modelCode=itemList.get(position).getModelCode();
        String modelName=itemList.get(position).getModelname();
        String title=itemList.get(position).getTitle();
        String pincode=itemList.get(position).getCustomerPinCode();
        String FirstName=itemList.get(position).getFirstName();
        String LastName=itemList.get(position).getLastName();
        String CustomerAlternateNumber=itemList.get(position).getCustomerAlternateNumber();
        String CustomerPhNo=itemList.get(position).getCustomerPhNo();
        String CustomerEmail=itemList.get(position).getCustomerEmail();
        String HouseNo=itemList.get(position).getHouseNo();
        String StreetName=itemList.get(position).getStreetName();
        String Landmark=itemList.get(position).getLandmark();
        String saledate=itemList.get(position).getSaledate();
        String Rate=itemList.get(position).getRate();



        if (tokenno.equals("")||tokenno.equals("0")){
            Intent intent=new Intent(DummyUpdationActivity.this,DummySaleUpdationFormActivity.class);
            intent.putExtra("temptoken",tempno);
            ...
            intent.putExtra("CustomerAlternateNumber",CustomerAlternateNumber);
            intent.putExtra("CustomerPhNo",CustomerPhNo);
